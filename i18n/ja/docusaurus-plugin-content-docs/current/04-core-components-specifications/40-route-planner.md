...
経路プランナーは、ユーザーの移動意図（`org`、`dst`、および `dept` または `arrv`）を1つ以上の経路候補に変換します。各経路は順序付けされた区間（`leg`）で構成されます。
...
どちらの実装も同じインターフェースを提供します。移動元・目的地・時間制約が与えられると、`Route` オブジェクトのリストを返します。
...
- `(org, dst, dept, arrv)` が与えられると、プランナーはネットワークから可能なパスを探索します。
...
- 移動元、目的地、出発時刻または到着時刻指定を含む計画リクエストをOTPエンドポイントに送信します。
...
- 両プランナーは同じインターフェースを実装します：`plan(org, dst, dept, arrv)` → `List[Route]`。
- すべての位置情報はWGS84座標（緯度/経度）を使用します。

:::warning
`SimplePlanner` は現在、出発時刻指定のルーティングのみをサポートしています。到着時刻指定のルーティングは将来のリリースで追加される予定です。
:::
